...
private:
	Orderqueue queueStation; //A new instance of Orderqueue class
	
	
	int currentTime;// The total time of order currentlly in the queue
	int currentItem;// The orders currentlly in the queue
	
	int totalTime;  // The total time of all orders being
					// processed
	
	int totalItem; // The total items of all orders being
				   // processed
	
	void setTime(); // The total time of finishing 
					// current saved Order
	
	void AddOrder(Order Aorder); // Enqueue a new Order into the queue
	
	void ReduceTime(int time_units) //The time when the Order is processing 
									//in the FetchStation
