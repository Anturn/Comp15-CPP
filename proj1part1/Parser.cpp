...
Order Parser::read_order() {
        ...
        /* 
         * order chaced_order.id arrived at
         * time chaced_order.arrival_timestamp
         */
        orders >> s >> chaced_order.id >> s >> s >> s
               >> chaced_order.arrival_timestamp;

        /* fetch time chaced_order.fetch_duration minutes, */
        orders >> s >> s >> chaced_order.fetch_duration >> s;

        /* pack time chaced_order.pack_duration minutes */
        orders >> s >> s >> chaced_order.pack_duration >> s;

        chaced_order.fetch_time_left = chaced_order.fetch_duration;
        chaced_order.pack_time_left  = chaced_order.pack_duration;
